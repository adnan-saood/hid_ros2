...
2. Install the package by calling `colcon build --symlink-install --packages-select dummy_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.b-robotized.com) than instead of the previous three steps, use `cb dummy_description` command.

Now, launch description test:
```
ros2 launch dummy_description view_dummy.launch.xml
```
or
```
ros2 launch dummy_description view_dummy.launch.py
```
